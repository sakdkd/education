...
include('../db_class/hr_functions.php');
 ...
 ?>
 <select  name="subjectid" id="subjectid" class="form-control" onChange="setval(this.value)"   >
                <option value="">Select Subject </option>
                
                <?php 
				$ds=mysqli_query($conn,"SELECT * FROM `levelsubjects` where `level_id`='$id' and `status`='1' and `view`='1' order by `id` desc " ); 
