...
//}
//elseif($mainActiveques<$question_total)
//{
...
-->
            ...
            <div class="next-prev-question d-flex justify-content-end">
                    ...
                    <?php if($question_total==1){?>
                    <button type="button" class="btn btn-success" onclick="questionvisibility(2)" disabled id="next_btn">Next Question</button>
                    <?php }else{?>
...
<div class="come-back-btn"> <a href="#" id="comback<?=$q_div;?>" onClick="Show_come_back()"><i class="fa fa-flag"></i> come back later</a></div>
                   </div>
                       </div>
                   </div>
                   </div>
...
<input name="mainActiveques" id="mainActiveques" type="hidden" value="<?php echo $displayques;?>">                       <input name="test_name" id="" type="text" value="<?php echo $test_name;?>">
	...
	//echo "select * from `questions` where `topic_id` in ($topic_imploded_string) and `status`='1' and `view`='1' limit 0,$question_total";
			...
			while($questionset=mysqli_fetch_array($question_query))
				{
					$main_table_qids[]=$questionset['id'];
					
				}
				echo "select * from `assignquestion` where `prid`='$test_name'";
				$question_query1=mysqli_query($conn,"select * from `assignquestion` where `prid`='$test_name'");
			while($questionset1=mysqli_fetch_array($question_query1))
				{
					$main_table_qids1[]=$questionset['qid'];
					
				}
				
			}
			$q_div=0;
			print_r($main_table_qids1);
					//	print_r($main_table_qids); die;
